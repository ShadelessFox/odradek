...
public final class UITextureToTextureConverter
    extends BaseTextureConverter<DS2.UITexture>
    implements Converter<DS2.UITexture, Texture, DS2Game> {

    @Override
    public Optional<Texture> convert(DS2.UITexture object, DS2Game game) {
        if (object.animated()) {
    ...
    private static Optional<Texture> convertTextureFrames(DS2.UITextureFramesInfo object) {
    ...
    private static Optional<Texture> convertTexture(DS2.TextureInfo object) {
        var format = mapFormat(object.header().pixelFormat().unwrap()).orElse(null);
        if (format == null) {
            return Optional.empty();
        }

        assert object.header().type() == DS2.ETextureType._2D;
    ...
    private static Duration mapFrequency(DS2.EUpdateFrequency frequency) {
