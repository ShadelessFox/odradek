...
public final class PaginatedTreeModel extends DelegateTreeModel {
    ...
    public PaginatedTreeModel(int limit, AbstractTreeModel model) {
        super(model);
        this.limit = limit;
    }
    ...
    public boolean handleNextPage(Object node, boolean loadFully) {
        if (isNextPage(node)) {
    ...
    public boolean isNextPage(Object node) {
        return node instanceof ExpanderNode;
    }

    public void loadNextPage(Object node, boolean loadFully) {
    ...
    private static PaginatedNode unwrap(Object node) {
        return (PaginatedNode) node;
    }

    private PaginatedNode[] getPathToRoot(PaginatedNode node) {
        return getPathToRoot(node, 0);
    }

    private PaginatedNode[] getPathToRoot(PaginatedNode node, int depth) {
    ...
    private record ExpanderNode(PaginatedNode parent, int total) {
        @Override
    ...
    private static class PaginatedNode implements TreeItem<Object> {
        ...
        public PaginatedNode(PaginatedNode parent, Object element, int loaded) {
            this.parent = parent;
